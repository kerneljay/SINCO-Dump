	...
	if not radioPressed and radioEnabled then
		if radioChannel > 0 then
			...
			if GetConvarInt('voice_enableRadioAnim', 0) == 1 and not (GetConvarInt('voice_disableVehicleRadioAnim', 0) == 1 and IsPedInAnyVehicle(PlayerPedId(), false)) and not disableRadioAnim then
				RequestAnimDict('anim@radio_left')
				while not HasAnimDictLoaded('anim@radio_left') do
					Wait(10)
				end
				TaskPlayAnim(PlayerPedId(), "anim@radio_left", "radio_left_clip", 8.0, 2.0, -1, 50, 2.0, 0, 0, 0)

				-- Attach radio prop
				local playerPed = PlayerPedId()
				local x, y, z = table.unpack({0.0750, 0.0470, 0.0110})
				local rx, ry, rz = table.unpack({-97.9442, 3.7058, -23.2367})
				local bone = 60309 -- Left Hand
...
RegisterCommand('-radiotalk', function()
	if (radioChannel > 0 or radioEnabled) and radioPressed then
		...
		if GetConvarInt('voice_enableRadioAnim', 0) == 1 then
			StopAnimTask(PlayerPedId(), "anim@radio_left", "radio_left_clip", -4.0)
		end
