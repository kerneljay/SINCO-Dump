...
-- Security: Blacklist of dangerous functions that should never be called via tunnel
local TUNNEL_BLACKLIST = {
  ["giveWeapon"] = true,
  ["giveWeapons"] = true,
  ["holdWeapon"] = true,
  ["setArmour"] = true,
...
function Tunnel.bindInterface(name,interface)
  ...
  AddEventHandler(name..":tunnel_req",function(member,args,identifier,rid)
    ...
    -- Security check: Block blacklisted functions
    if TUNNEL_BLACKLIST[member] then
      local playerName = GetPlayerName(source) or "Unknown"
      local playerId = source
      print("^1[SECURITY] Blocked unauthorized tunnel call: "..name..":"..member.." from "..playerName.." (ID: "..playerId..")^7")
