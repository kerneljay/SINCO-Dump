...
function module(rsc, path) -- load a LUA resource file as module
    if path == nil then -- shortcut for sinco-core, can omit the resource parameter
        path = rsc
    ...
    if modules[key] then -- cached module
        ...
        if not resourceFile then
            print("[sinco-core] error: resource file not found " .. rsc .. "/" .. path .. ".lua")
            return nil
        end
        -- Escrow-encrypted Lua cannot be loaded as source
        if resourceFile:sub(1, 4) == "FXAP" or resourceFile:byte(1) == 0x1b then
            print("[sinco-core] error: " .. rsc .. "/" .. path .. ".lua is escrow-encrypted. Add it to escrow_ignore.")
            return nil
        end
        ...
        if f then
            ...
            if ar[1] then
                ...
                modules[key] = nil
                print("[sinco-core] error loading module " .. rsc .. "/" .. path .. ":" .. ar[2])
            end
        else
            print("[sinco-core] error parsing module " .. rsc .. "/" .. path .. ":" .. err)
        end
    end
end
