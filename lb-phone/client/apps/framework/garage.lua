...
local function FindCar(plate)
    ...
    local wanted = string.sub((tostring(plate or ""):gsub("%s+", "")):upper(), 1, 8)

    for i = 1, #vehicles do
        local vehicle = vehicles[i]

        if DoesEntityExist(vehicle) then
            local current = string.sub((GetVehicleNumberPlateText(vehicle):gsub("%s+", "")):upper(), 1, 8)
...
RegisterNUICallback("Garage", function(data, cb)
    ...
    if action == "getVehicles" then
        ...
        for i = 1, #cars do
            if cars[i].label and cars[i].label ~= "" then
                cars[i].model = cars[i].label
            else
                cars[i].model = GetVehicleLabel(cars[i].model)
            end
            if GetResourceState("sinco-core") == "started" and exports["sinco-core"]:isPhoneVehicleOut(cars[i].spawncode, cars[i].plate) then
                cars[i].location = "out"
            end
        end
    ...
    elseif action == "valet" then
        ...
        if GetResourceState("sinco-core") == "started" and exports["sinco-core"]:isPhoneVehicleOut(data.spawncode, data.plate) then
